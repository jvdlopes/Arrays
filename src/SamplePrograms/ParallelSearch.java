...
public class ParallelSearch {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < product.length; i++) {
            System.out.format("%35s%10.2f%20s\n", product[i], price[i], category[i]);
        }
        System.out.println("\nPRODUCT SEARCH\n");
        ...
        int loc = search(prod);
        if(loc==-1)
            System.out.println(prod + " not found in database");
        else{
            System.out.println(prod + "price $" + price[loc]);
            System.out.println(prod + " category " + category[loc]);
        }
    }
